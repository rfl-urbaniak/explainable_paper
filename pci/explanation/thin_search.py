...
from pci.explanation.utils import (
    ...
    lean_get_alternative_sample,
    sample_k_indices,
)
from pci.structured.schemata import Norm
from pci.structured.structured_model import StructuredModel
from pci.tools.data_processing import get_variable_from_structured_data
from pci.tools.rsample import Exogenate, RSampleNormalSites
...
class LeanSearchSampler(pyro.nn.PyroModule):
    """
    ...
    def __init__(
        ...
        for attr in [
            ...
            setattr(self, attr, getattr(searchable_model, attr))

        self.outcome_variable = searchable_model.outcome_variable
        ...
        self.structured_model = cast(
            StructuredModel, searchable_model._structured_model
        )

        assert self.structured_model.model_iterable is not None
        self.upstream_model = self.structured_model.model_iterable[0]
        ...
        self.suspects = [
            s
            for s in searchable_model.suspects
            if s not in searchable_model.deterministic_sites
        ]

        if self.witness_sites is None:
            self.witnesses = [
                s
                for s in searchable_model.sites_of_interest
                if s
                not in searchable_model.deterministic_sites + [self.outcome_variable]
            ]

        else:
            self.witnesses = [
                s
                for s in searchable_model.witness_sites
                if s
                not in searchable_model.deterministic_sites + [self.outcome_variable]
            ]
    ...
    def sample(
        self,
        factual_features: dict[str, dict[str, torch.Tensor]] | dict[str, torch.Tensor],
        num_samples: int = 3,
        ...
        # get factual dictionary from possibly nested factual features
        factual_is_nested = (
            isinstance(factual_features, dict)
            and "continuous" in factual_features
            and "categorical" in factual_features
        )
        if factual_is_nested:
            factual_dictionary = {
                key: get_variable_from_structured_data(
                    factual_features, key, aliases=self.aliases
                )
                for key in set(self.sites_of_interest) | set(self.hard_evidence_sites)
            }

        else:
            factual_dictionary = {
                key: factual_features[
                    self.aliases[key]
                    if self.aliases is not None and key in self.aliases.keys()
                    else key
                ]
                for key in set(self.sites_of_interest) | set(self.hard_evidence_sites)
            }
        ...
        for sample_index in tqdm(range(num_samples)):
            ...
            active_witnesses = self.hard_evidence_sites + deduped_active_witnesses
            ...
            witness_data = witness_interventions.copy()

            attempt = 0

            while attempt < max_resample_attempts:
                try:
                    alternative_interventions = lean_get_alternative_sample(
                        self.upstream_model,
                        active_antecedents=active_antecedents,
                        n_size=batch_size,
                        factual_dictionary=factual_dictionary,
            ...
            _uk = getattr(self.upstream_model, "kernel_modules", None) or getattr(
                self.upstream_model, "kernels", {}
            )
            normal_distribution_schemata = {
                k: _uk[k].outcome_schema
                for k in _uk.keys()
                if isinstance(
                    next(iter(_uk[k].outcome_schema.values())),
                    Norm,
                )
            }

            normal_sites = list(normal_distribution_schemata.keys())

            with torch.no_grad():
                with Exogenate():
                    with do(
                        actions=antecedent_sufficiency_interventions
                        | witness_interventions
                    ):
                        with condition(data=witness_data):
                            with pyro.poutine.trace() as tr_sufficiency_world:
                                with RSampleNormalSites(*normal_sites):
                ...
                shared_noise = {
                    name: node["value"]
                    for name, node in tr_sufficiency_world.trace.nodes.items()
                    if name.endswith("_u") and "value" in node
                }
