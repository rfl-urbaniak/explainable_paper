...
Computing these probabilities: $\pns(\varname{gender}=\text{male}, \varname{loan}=F) \approx 0.01$, $\pns(\varname{gender}=\text{female}) \approx 0.39$, and $\pns(\varname{credit}=\text{bad}) \approx 0.52$.
...
The remaining failures (DYM, DMX, DMXY) trace to two limitations neither method
addresses: prediction models that include non-causal features~(DYM, DMX), and the
inability of Shapley averaging to distinguish direct from indirect causal paths~(DMXY).
