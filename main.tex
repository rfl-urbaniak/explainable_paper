...
Fix a probabilistic structural causal model
$M=\langle \mathbf{U},\mathbf{V},\mathbf{F},P_{\mathbf{U}}\rangle$
...
failure reflects that without a witness holding \varname{check\text{-}failed} fixed,
...
because $P(\varname{loan}=1\mid\varname{gender}=\text{female},\varname{credit}=\text{bad})=0$,
...
We inspect these limitations through two examples. We first return to the OBCB model
introduced in Section~\ref{sec:motivations}: it is simple enough to permit exact analytic
computation and the reader already knows the desiderata, making it a concise warm-up that
exposes the key failure modes directly. We then examine a continuous mediation example in
depth, tracing each limitation more carefully and comparing plain SHAP, Causal SHAP
\citep{heskes2020causal}, and PCI.
...
\subsection{OBCB revisited: plain SHAP}\label{sec:obcb_shap}

We apply SHAP to $g(x)=P(\varname{loan}=0\mid x)$ for Alice and Bob from
...
\varname{credit} gets a higher responsibility magnitude than \varname{gender}
($0.174 > 0.107$). This is an overdetermination effect: \varname{credit}=bad
correlates with the \varname{check\text{-}failed} mechanism that is the proximate
cause of Alice's denial, so SHAP's observational marginalization absorbs this
correlation into $|\phi^g_{\varname{credit}}|$, inflating it above
$|\phi^g_{\varname{gender}}|$. PCI conditions on the factual
...
We work with the following \textbf{generative model}, where $X$ sends a signal,
$M$ mediates it with some noise, and $Y$ is the final outcome, which is also noisy:
\begin{align}
