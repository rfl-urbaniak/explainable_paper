...
\medskip\noindent The absence of a sufficiency clause in Halpern's definition is exactly
what a separate strand of the actual-causality literature sets out to repair.
\citet{beckersSufficiency2021}, building on the counterfactual NESS account
\citep{beckersNESS2021}, make sufficiency central: a cause is a necessary element of a set
that is \emph{sufficient} for the outcome. \ourapproach's sufficiency component --- the
$Y^s$ factor of Definition~\ref{def:jointnecsuf}, which restores a candidate set
$\mathbf{C}$ to factual values, holds the witnesses $\mathbf{T}$ fixed, and checks that the
outcome persists --- is the graded, probabilistic analogue of that condition: where the
NESS test returns set membership, the joint measure returns a $\Gamma$-weighted degree, and
the witness set supplies the held-fixed context that NESS sufficient sets leave implicit.
The necessity-only correspondence below is thus the AC-facing half of a construction whose
sufficiency half answers to Beckers' program rather than to Halpern's.
...
of the pair $(M,\Delta)$, not of $\Gamma$ (we abbreviate $N(\mathbf{C},\mathbf{T})$ to
$N(\mathbf{C})$ when the witness argument plays no role). It says that adding suspects to a candidate
