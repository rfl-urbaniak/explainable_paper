\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/structure_map_04_actual_causality.pdf}
  \caption{\textbf{Roadmap: relating to actual causality} (current stage). The next
  section establishes the formal correspondence between the \ourapproach kernel and
  Halpern's actual cause. It then positions \ourapproach against Pearl's probability of
  actual causation (\cref{sub:pearl_actual_cause_prob}), before the empirical
  evaluation.}
  \label{fig:roadmap_ac}
\end{figure}
...
\emph{probability of actual causation}; and the empirical comparison (runtime
and verdict recovery as model size grows) is reported in \cref{sec:evaluation}.
...
not as a substantive sufficiency-attribution component. Its substantive role is
instead the graded, probabilistic analogue of Beckers' NESS test: $Y^s$ restores a
...
permits, not a limitation of \ourapproach{}.
...
the configuration is being asked about. Throughout the kernel and its proofs we write
...
first factor is the $\Gamma$-selection weight, positive exactly when
$(\mathbf{C},\mathbf{T})$ carries positive mass; the regularity we need is that this
weight never vanishes on a legal configuration.
...
The first observation is that there is a simplified setting in which a positive value of $\Phi$,
...
\int\sum_{\mathbf{T}}\Phi_{\mathbf{u}}(\mathbf{C},\mathbf{T})\,P_{\mathbf{U}}(d\mathbf{u})$, not knowing
