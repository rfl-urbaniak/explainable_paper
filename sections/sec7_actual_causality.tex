...
alternative satisfies $Y(\mathbf{c}')<y_0$ strictly rather than $Y(\mathbf{c}')=y_0$
exactly --- generic under a continuous pushforward, but not automatic, so a witness
...
The hypothesis on $\mathbf{S}$ is what rules out the following obstruction.
...
case); this is what lets the proof below invoke Theorem~\ref{th:ac-exp} on
...
$N(\mathbf{C})$ for $N(\mathbf{C},\mathbf{T})$ where this does not depend on
$\mathbf{T}$ at all --- i.e.\ pinning any admissible witness set at its factual
