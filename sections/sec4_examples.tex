...
\subsection{PCI computation for the signal example}
\label{sec:pci_signal_computation}

We work at the factual instance $X^\star = M^\star = Y^\star = 1$,
which gives abducted noise $\varepsilon_M^\star = M^\star - X^\star = 0$
and $\varepsilon_Y^\star = Y^\star - M^\star = 0$.
The causal impact function is the \textbf{absolute difference score}
(Example~\ref{ex:absolute_score}):
\[
ci(y^s, y^n, y^\star) = |y^n - y^\star| - |y^s - y^\star|.
\]
Partial abduction: the factual noise is abducted in the sufficiency world;
fresh independent noise is drawn in the necessity world.
The alternative for the suspect is drawn from its marginal $P(A)$.
For target $B$, $\mathbf{S}$ is the full set of two input features of $B$
and $\mathbf{W}$ is the remaining input (neither suspect nor target);
$\Gamma_s$ is uniform over the three non-empty subsets of $\mathbf{S}$,
and $\Gamma_w$ is uniform over $2^{\mathbf{W}} = \{\emptyset, \{W\}\}$.

\paragraph{$R(X \rightsquigarrow Y)$.}
$\mathbf{S}=\{X,M\}$, $\mathbf{W}=\{M\}$, $w^\star=m^\star=1$,
$X'\sim P(X)=\mathcal{N}(0.5,\,0.25)$.
Rejection sampling from $\Gamma_s\times\Gamma_w$ accepts four of the six raw
$(\mathbf{C},\mathbf{T})$ pairs ($\Gamma$'s normalizer $Z=2/3$); each accepted
pair carries weight $p^\Gamma(\mathbf{C},\mathbf{T})=\tfrac{1}{4}$, and the
four sum to~$1$. Three of the four contain the suspect $X$ and enter
$P_X^{s,n}$; the fourth, $(\mathbf{C}{=}\{M\},\mathbf{T}{=}\emptyset)$,
carries the same weight $\tfrac{1}{4}$ but does not intervene on $X$, so it
contributes $0$ to the attribution. Per Definition~\ref{def:jointnecsuf},
PCI is the $\Gamma$-integral of $ci$ over the slice $\{X_k\in\mathbf{C}\}$
--- not a renormalized expectation --- so the three enumerated weights below
sum to $\tfrac{3}{4}$, not $1$.

\textit{Sufficiency} (all three pairs): abducted $\varepsilon_Y^\star=0$ gives
$Y^s=1=Y^\star$, so $|Y^s-1|=0$ throughout.
Sufficiency is maximally satisfied: the factual noise $\varepsilon_Y^\star=0$
means restoring $X$ to its factual value exactly recovers the outcome.

\textit{Necessity by case}:
\begin{align*}
(C{=}\{X\},\,T{=}\emptyset),\;\tfrac{1}{4}:
  &\quad \mathrm{do}(X{=}X');\;
    Y^n = X' + \varepsilon_M^n + \varepsilon_Y^n, \\
  &\quad Y^n-1\sim\mathcal{N}(-0.5,\,0.45),\;
    \mathbb{E}[|Y^n-1|]=0.677.\\[4pt]
(C{=}\{X\},\,T{=}\{M\}),\;\tfrac{1}{4}:
  &\quad \mathrm{do}(X{=}X',\,M{=}1);\;
    Y^n = 1+\varepsilon_Y^n,\;
    \mathbb{E}[|Y^n-1|]=\sqrt{0.2/\pi}\approx 0.252.\\[4pt]
(C{=}\{X,M\},\,T{=}\emptyset),\;\tfrac{1}{4}:
  &\quad \mathrm{do}(X{=}X',\,M{=}M');\;
    Y^n = M'+\varepsilon_Y^n, \\
  &\quad Y^n-1\sim\mathcal{N}(-0.5,\,0.45),\;
    \mathbb{E}[|Y^n-1|]=0.677.
\end{align*}
In the first and third cases $M$ propagates freely; only the second case
pins $M$ as a witness and severs $X$'s indirect path.
Combining:
\[
R(X \rightsquigarrow Y \mid \mathbf{W}{=}\{M\})
= \frac{0.677+0.252+0.677}{4} \approx 0.402.
\]

\paragraph{$R(Y \rightsquigarrow X)$.}
$\mathbf{S}=\{M,Y\}$, $\mathbf{W}=\{M\}$.
$X$ is exogenous with no parents, so any do-intervention leaves
$X^n=X^\star=1$ (necessity) and $X^s=X^\star=1$ (sufficiency) in every
$(C,T)$ configuration. Hence $ci=0$ for all pairs and
\[
...
\midrule
\textbf{DXY}  & $R(X \rightsquigarrow Y) > 0$
  & $0.250$\;\checkmark & $0.250$\;\checkmark
  & $0.452$\;\checkmark & $0.402$\;\checkmark$^\dagger$ \\
\textbf{DMY}  & $R(M \rightsquigarrow Y) > 0$
  & $0.250$\;\checkmark & $0.250$\;\checkmark
  & $0.452$\;\checkmark & $0.508$\;\checkmark \\
\textbf{DXM}  & $R(X \rightsquigarrow M) > 0$
  & $0.306$\;\checkmark & $0.375$\;\checkmark
  & $0.421$\;\checkmark & $0.473$\;\checkmark \\
\textbf{DYX}  & $R(Y \rightsquigarrow X) = 0$
  & $0.139$\;\texttimes & $0.000$\;\checkmark
  & $0.000$\;\checkmark & $0.000$\;\checkmark \\
\textbf{DYM}  & $R(Y \rightsquigarrow M) = 0$
  & $0.194$\;\texttimes & $0.125$\;\texttimes
  & $0.000$\;\checkmark & $0.000$\;\checkmark \\
\textbf{DMX}  & $R(M \rightsquigarrow X) = 0$
  & $0.219$\;\texttimes & $0.357$\;\texttimes
  & $0.000$\;\checkmark & $0.000$\;\checkmark \\
\textbf{DMXY} & $R(M \rightsquigarrow Y) > R(X \rightsquigarrow Y)$
  & $0.25{=}0.25$\;\texttimes & $0.25{=}0.25$\;\texttimes
  & $0.452{=}0.452$\;\texttimes & $0.508{>}0.402$\;\checkmark$^\ddagger$ \\
\bottomrule
\end{tabular}
...
PCI values are $\mathbb{E}[|y^n - B^\star|]$; $\mathbf{S}$ = both input
features of $B$, $\Gamma_s$/$\Gamma_w$ uniform.
\textbf{W=3rd}: the third variable (neither $A$ nor $B$) is in $\mathbf{W}$
and sampled as an active witness when not in $C$.
$^\dagger$~0.402 weights three suspect-containing $(C,T)$ configurations at
...
witness (contribution~0.252, $X$'s path severed) and two let $M$ propagate
freely (contribution~0.677 each).
$^\ddagger$~The two values use different witness sets ($\mathbf{W}{=}\{M\}$
for DXY, $\mathbf{W}{=}\{X\}$ for DMY), so the comparison is illustrative
rather than a single-game ranking.}
...
$\mathrm{PCI}(M\to Y\mid\mathbf{W}{=}\emptyset)$ equal $0.452$, because the
path variance reaching $Y$ is $\mathrm{Var}(X)+\mathrm{Var}(\varepsilon_M)+\mathrm{Var}(\varepsilon_Y)=0.45$
for $X$ (through $M$) and $\mathrm{Var}(M)+\mathrm{Var}(\varepsilon_Y)=0.45$ for $M$
(directly) --- an exact tie across every suspect coalition $C$,
so the $\Gamma_s$-weighted average is also tied.
...
Admitting the third variable as a witness resolves this.
$\mathrm{PCI}(X\to Y\mid\mathbf{W}{=}\{M\})\approx 0.402$ weights three
suspect-containing $(C,T)$ configurations at $1/4$ each: in two cases $M$
propagates freely (contributing~$0.677$ each) and in one case $M$ is held
fixed as witness, severing $X$'s indirect path (contributing~$0.252$).
$\mathrm{PCI}(M\to Y\mid\mathbf{W}{=}\{X\})\approx 0.508$ weights three cases
in which $M$ takes an alternative value; since $X$ does not appear in $Y$'s
structural equation, pinning $X$ as a witness has no effect on $Y^n$, and all
three cases contribute~$0.677$.
The gap $0.508>0.402$ satisfies DMXY: the witness mechanism selectively
reduces $X$'s attribution (via the $M$-witness case) while leaving $M$'s
unchanged.
The comparison relies on different witness sets for the two cells, so it is
...
two sources. Prediction models include non-causal features that the
conditioning distribution cannot exclude~(DYM, DMX). Shapley averaging cannot
distinguish direct from indirect causal paths~(DMXY). PCI addresses both
through the structural intervention framework and the witness mechanism.
