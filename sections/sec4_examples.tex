...
In this section we adopt the SHAP literature's standard notation: $S \subseteq N$ denotes a
coalition (a subset of features held to their factual values during evaluation), playing the same
role here that $\mathbf{C} \subseteq \mathbf{S}$ played in Section~\ref{sec:causal_impact}. We
retain the literature's symbol throughout for ease of cross-reference; the bold $\mathbf{S}$
refers exclusively to PCI's full suspect set.
...
$\mathbb{E}[f(\mathbf{X}_S = \mathbf{x}_S, \mathbf{X}_{\bar{S}})]$, marginalising over the
...
the PSCM produces under the intervention $do(X_S = x_S^\star)$, so the dropped
...
$\varname{check\text{-}failed}$ are dropped at $S\in\{\emptyset,\{\varname{gender}\}\}$,
the first two NaN
...
$S=\{\varname{gender},\varname{check\text{-}failed}\}$. Both hold
...
PCI values are signed $\bar c = \mathbb{E}[|B^n-B^\star|] - \mathbb{E}[|B^s-B^\star|]$;
$\mathbf{S}$ = both input features of $B$, with $\Gamma_s$ uniform over
non-empty subsets of $\mathbf{S}$ and $\Gamma_w$ uniform over $2^{\mathbf{W}}$,
rejecting draws with $\mathbf{C}\cap\mathbf{T}\neq\emptyset$.
\textbf{W=3rd}: the third variable (neither $A$ nor $B$) is in $\mathbf{W}$
and sampled as an active witness when not in $\mathbf{C}$; note that this
variable lies in both $\mathbf{S}$ and $\mathbf{W}$, with the rejection step
ensuring it never plays both roles in the same draw.
...
for D-XY, $\mathbf{W}{=}\{X\}$ for D-MY), so the comparison is illustrative
...
which the witness mechanism turns into a robust margin.
