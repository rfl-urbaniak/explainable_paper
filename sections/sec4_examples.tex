\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/structure_map_03_examples_shap.pdf}
  \caption{\textbf{Roadmap: positioning against SHAP} (current stage). With
  \ourapproach defined, the next section compares it with SHAP, Causal SHAP, ATE, and
  CATE on two worked examples. The argument then establishes the formal relation to
  actual causality (\cref{sec:relation_with_ac}) and to Pearl's probability of actual
  causation, before the empirical evaluation.}
  \label{fig:roadmap_examples}
\end{figure}
...
(\cref{sec:shap_defs}), then re-run OBCB
...
Formal definitions of both are given in \cref{sec:shap_defs}.

Two recent methods come even closer, since they too build attribution on Pearl's
probabilities of necessity and sufficiency. FANS \citep{chenFANS2024} explains a
prediction by searching a small region around the input (a ball of radius $b$ in the
perturbed coordinates) for the largest PNS attainable there, estimating the
counterfactual probabilities by resampling. LEWIS \citep{galhotraLewis2021} works from a
causal graph and input--output data, computing necessity, sufficiency, and combined
scores for a black-box decision, typically at the population or subgroup level, and turns
them into recourse. Both are well suited to their stated aims, explaining and providing
recourse for opaque models under limited assumptions, and both share our starting point
...
Second, we average rather than optimise for a best case, both over the model's noise and
over candidate cause sets. FANS reports the maximum PNS over the local regions it searches
...
case of $\Gamma$. The resulting score is the feature's probability of causation in the
case at hand, and it does not depend on a region- or subset-search procedure.
...
witnesses, and the OBCB and signal walkthroughs below turn on exactly this mechanism.
...
deliberate trade: as noted in \cref{sec:intro}, LEWIS recovers its scores from a graph and
observational data at the population level, whereas \ourapproach{} assumes a fully
specified model and returns a value for the individual case.
...
explain a model's predicted output's departure from the same model's output averaged over the population, not the individual
...
PCI against the desiderata of Section~\ref{sec:causal_impact} and some further desiderata related to the example we
...
disconnected from the loan) rather than merely integrating over its noise. In
...
\crefrange{eq:X}{eq:Y}. That one missing edge is what desideratum \textbf{D-MXY}
asks an attribution to register: $M$ acts on $Y$ directly, whereas $X$ acts only
at one remove.}
...
\end{align*}
\noindent The direction of the inequality is a convention rather than a law of
responsibility; what matters is that the method separates the direct from the
indirect path at all, and scoring the closer cause higher is one principled,
consistent way to register that separation. We apply this convention throughout.
...
\noindent\textbf{D-MX: the interventional correction reallocates rather than removes the spurious attribution.}
...
$0.319-0.186=0.133$ at $\mathbf{W}=\{$third$\}$: the ordering is now
\emph{robust} rather than marginal.
...
\Ourapproach in Section~\ref{sec:causal_impact}. PCI addresses these gaps
...
attribution --- is summarised in \cref{sec:evaluation} and detailed in
