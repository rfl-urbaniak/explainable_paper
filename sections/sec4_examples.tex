...
\]
Both worlds run the SCM with fresh exogenous noise drawn from $P_{\mathbf{U}}$:
the necessity world replaces the suspect with $A'\sim P(A)$ and propagates,
the sufficiency world restores the suspect to $a^\star$ and propagates.
...
For each suspect-containing pair we compute the necessity expectation
$\mathbb{E}[|Y^n-1|]$ and the sufficiency expectation $\mathbb{E}[|Y^s-1|]$
in closed form via the folded-normal formula, then take the signed
difference $\bar c = \mathbb{E}[|Y^n-1|] - \mathbb{E}[|Y^s-1|]$.

\textit{Per-pair contributions}:
\begin{align*}
(C{=}\{X\},\,T{=}\emptyset),\;\tfrac{1}{4}:
  &\quad \mathrm{do}(X{=}X'):\;
    Y^n = X' + \varepsilon_M^n + \varepsilon_Y^n, \\
  &\quad \mathrm{do}(X{=}1):\;
    Y^s = 1 + \varepsilon_M^s + \varepsilon_Y^s, \\
  &\quad \mathbb{E}[|Y^n-1|]=0.677,\;
    \mathbb{E}[|Y^s-1|]=\sqrt{0.4/\pi}\approx 0.357,\;
    \bar c \approx 0.320.\\[4pt]
(C{=}\{X\},\,T{=}\{M\}),\;\tfrac{1}{4}:
  &\quad \mathrm{do}(X{=}X',\,M{=}1):\;
    Y^n = 1+\varepsilon_Y^n, \\
  &\quad \mathrm{do}(X{=}1,\,M{=}1):\;
    Y^s = 1+\varepsilon_Y^s, \\
  &\quad \mathbb{E}[|Y^n-1|]=\mathbb{E}[|Y^s-1|]=\sqrt{0.2/\pi}\approx 0.252,\;
    \bar c = 0.\\[4pt]
(C{=}\{X,M\},\,T{=}\emptyset),\;\tfrac{1}{4}:
  ...
  &\quad \mathrm{do}(X{=}1,\,M{=}1):\;
    Y^s = 1+\varepsilon_Y^s, \\
  &\quad \mathbb{E}[|Y^n-1|]=0.677,\;
    \mathbb{E}[|Y^s-1|]=0.252,\;
    \bar c \approx 0.425.
\end{align*}
The witness pair $(C{=}\{X\},\,T{=}\{M\})$ contributes 0: pinning $M$ at
$m^\star$ in both worlds equalizes their distributions. The other two
pairs let $M$ propagate freely. Combining:
\[
R(X \rightsquigarrow Y \mid \mathbf{W}{=}\{M\})
= \tfrac{1}{4}\bigl(0.320 + 0 + 0.425\bigr) \approx 0.186.
\]

\paragraph{$R(Y \rightsquigarrow X)$.}
$\mathbf{S}=\{M,Y\}$, $\mathbf{W}=\{M\}$.
$X$ is exogenous: no do-intervention on $\{M,Y\}$ alters $X$'s structural
equation. The necessity and sufficiency distributions of $X$ coincide (both
draw $X$ from its marginal $P(X)$), so $ci=0$ for every $(C,T)$ pair and
\[
...
Causal SHAP, and PCI (alternative $\sim P(A)$, prior-noise sufficiency) on the
signal with mediation example ($X\!\to\!M\!\to\!Y$), instance $X=M=Y=1$.
PCI values are signed $\bar c = \mathbb{E}[|Y^n-B^\star|] - \mathbb{E}[|Y^s-B^\star|]$;
$\mathbf{S}$ = both input features of $B$, $\Gamma_s$/$\Gamma_w$ uniform.
...
DYX, DYM, and DMX are zero exactly: intervening on a variable that does not
...
DXY, DMY, and DXM are positive.
DMXY also \textbf{passes} at $\mathrm{PCI}(M\to Y)\approx 0.283 > \mathrm{PCI}(X\to Y)\approx 0.249$,
...
$0.319-0.186=0.133$ at $\mathbf{W}=\{$third$\}$ --- DMXY is now a
...
the structural intervention framework already produces a structural gap on
DMXY through path-noise asymmetry in the sufficiency world; the witness
mechanism then turns that gap into a robust margin.
