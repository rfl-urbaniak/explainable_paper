...
With \ourapproach now defined (Section~\ref{sec:causal_impact}), this section
previews what the rest of the paper establishes: how \ourapproach compares against
the dominant feature-attribution baselines, against actual causality and Pearl's
probability of causation, against a causal-effect attribution method, and how it
behaves as the model grows from textbook size to a deployed system. Each result
appears here at a glance --- one claim, one figure, one headline number --- with a
pointer to where it is developed in full: the formal comparisons in
...
synthetic structural model that carries three archetypes at once --- linear
necessary-and-sufficient, overdetermined, and preempted (gated) --- plus an
irrelevance control, all with an analytic ground truth
...
\label{sec:eval_shap}
\ourapproach separates direct and indirect causal contributions where SHAP-based
attribution cannot. Both plain and Causal SHAP decompose a prediction's departure
from a population mean rather than the realized outcome, which produces two failures
at once. First, they assign attribution backward, against the arrows of the causal
graph, failing desiderata D-YX, D-YM, and D-MX; Causal SHAP's interventional
correction repairs only D-YX. Second, they tie the direct and indirect causes of an
outcome (D-MXY). \ourapproach{}'s realized-outcome reference and witness mechanism
break both symmetries, separating the direct cause from the indirect one at every
instance. Section~\ref{sec:shap_examples} works through this in full on two worked
...
\label{sec:eval_dce}
The paper's next comparison targets a causal-effect attribution method: the
...
tracking the realized-outcome contrast rather than a point derivative keeps
\ourapproach's attribution aligned with each feature's structural role.
