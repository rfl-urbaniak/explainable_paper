...
Under a setting $\mathbf{U} = \mathbf{u}$ of the exogenous variables, $M$ produces $S = s^\star$ and $Y = y^\star$, denoted $(M, \mathbf{u}) \models S=s^\star, Y=y^\star$. We read $\models$ as ``yields'' (the model under noise $\mathbf{u}$ makes the stated equalities true), and its crossed form $\not\models$ as ``does not yield'' (the stated event fails to hold).
...
Conditioning is not a workaround: we saw with $\pns_c$ that context fixed by observation does not survive the subsequent intervention, since the credit intervention propagates downstream and overrides the $\varname{check\text{-}failed}$ value that conditioning had fixed.
...
For Alice, the witness sets that drive the attribution are those containing $\varname{check\text{-}failed}$: when this variable is held fixed at $F$ --- the value recording that the check was never performed --- flipping gender to male leads to loan approval.
...
The witness sets that contain $\varname{check\text{-}failed}{=}F$ now work against credit: holding that mediator fixed at $F$ disconnects credit from the loan decision, because the bank's policy is to reject unchecked applicants regardless of credit.
...
\ourapproach{} in the existing literature, so the later comparison does not come out
of nowhere: Pearl's \emph{probability of actual causation}
\citep[Def.~10.3.5]{causalityPearl}, the posterior probability --- given the observed
evidence --- that the noise lands in a state for which the actual-cause verdict holds.
Because \ourapproach{} proposes a seemingly similar bridge from structural information
to a probabilistic score, and the two give the same rankings in typical simple cases,
we defer a precise side-by-side comparison to
Section~\ref{sub:pearl_actual_cause_prob}, once enough machinery is on the page. (The
formula and its Bayes-rule reading are stated there, in Eq.~\eqref{eq:pearl-paac}.)
