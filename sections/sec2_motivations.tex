...
The central difficulty in causal attribution is not computing counterfactuals: it is knowing which counterfactuals to compute. For example, consider two questions: the \emph{type} question (does $X$ tend to cause $Y$?) and the \emph{token}, or actual-causation, question (was $X$'s contribution live in the specific situation we are explaining?). Methods built to answer the former routinely miss the latter: a cause can be genuine in general, in that it does produce the outcome in other situations, yet have had its causal path inactive in the particular case at hand, so that it made no difference to what actually happened. This is the gap the running example below makes concrete.
...
Under the but-for clause for single cause candidates, neither her gender nor her credit score is responsible.
...
not properties reverse-engineered to flatter \ourapproach{}, so PN, PS, PNS, SHAP,
and Causal SHAP are held to the same independently motivated standard below and in
