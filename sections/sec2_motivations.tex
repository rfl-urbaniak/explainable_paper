...
$(M,\mathbf{u})$ just in case there is an active witness set
$\mathbf{T}\subseteq\mathbf{W}$ disjoint from $\mathbf{C}$, at its factual
setting $\mathbf{T}=\mathbf{t}^\star$, such that
\begin{align*}
&(M,\mathbf{u})\models \mathbf{C}=\mathbf{c}^\star,\;
                       \mathbf{W}=\mathbf{w}^\star,\; Y=y^\star, \\
&(M,\mathbf{u})\bigl[\mathbf{C}\leftarrow\mathbf{c}',\;
                     \mathbf{T}\leftarrow\mathbf{t}^\star\bigr]
  \not\models Y=y^\star
\end{align*}
for some $\mathbf{c}'$ distinct everywhere from $\mathbf{c}^\star$, where no
proper subset of $\mathbf{C}$ satisfies these conditions.
\end{definition}
