...
There are principled probabilistic theories of type-level causal strength --- \citet{sprenger2018foundations}, for instance, derives a difference-making measure $p^*(E \mid C) - p^*(E \mid \neg C)$ axiomatically --- but they deliberately bracket the token-level question of which cause was live in the case at hand, and amalgamate over mediators rather than tracking them, which is exactly the context-sensitivity we are after.
...
The counterfactual and production traditions thus converge, so the attribution is not an artifact of one chosen formalism.
...
\noindent Fixing the mediator $\varname{check\text{-}failed}$ at its factual value exposes
...
pool $\mathbf{S}$ and witness pool $\mathbf{W}$. Building on the modified definition --- whose witnesses
are held at their \emph{actual} values --- rather than the original Halpern--Pearl definition is a
deliberate design choice that \ourapproach inherits; it is also where we depart from prior probabilistic
treatments of actual causation, which extend the original definition
(Section~\ref{sec:other-prob-ac}).
...
quantities rather than a single binary verdict.
...
But this averages an all-or-nothing verdict over contexts: it inherits actual
causality's binary reading and its intractability rather than removing them.
...
\citep{pearl1999probabilities,tian2000probabilities}, which we introduce in this
...
\paragraph{The same limitation affects ATE and CATE.}
