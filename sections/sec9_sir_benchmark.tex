...
population. Note that a policy can \emph{raise} this probability relative to
...
with both policies, $0.84$ with lockdown only, and $0.81$ with mask only ---
the two single-policy regimes are statistically indistinguishable by
...
a cloud displaced toward smaller $y_{\mathrm{nec}}$ at fixed $y_{\mathrm{suff}}$
--- the \emph{cause signature} on this geometry --- whereas a non-cause hugs the
...
the lockdown cloud's displacement below the diagonal toward smaller
...
at the start of this appendix --- lockdown is the proximate cause of excessive
overshoot and masking only a context-sensitive one, despite both policies
appearing similar under but-for analysis --- and expresses them on a
