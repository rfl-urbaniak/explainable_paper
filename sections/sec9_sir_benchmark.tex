...
so, since a higher overshoot probability marks a \emph{weaker} policy, mask-only
is the weakest of the three intervention regimes.
...
The suspects are $\{\mathrm{lockdown},\mathrm{mask}\}$. On each draw, the
sampler draws witnesses from the policy efficiencies
$\{\mathrm{lockdown\_efficiency}, \mathrm{mask\_efficiency},
\mathrm{joint\_efficiency}\}$, excluding the suspect itself from the
witness set. The factual world fixes both policies on
($\mathrm{lockdown}=1, \mathrm{mask}=1$), so the sampler reads the factual
outcome $y^\star=\mathrm{overshoot}$ from the same trajectory. For each
...
whether the verdict survives holding that variable fixed at factual versus
leaving it free. We replicate the experiment by partitioning the
\ourapproach regimes by whether the sampler drew the partner-policy
efficiency variable into the witness set
...
world reported earlier, so we do not expect the fixed/free scores to average
...
adapt. \ourapproach thus expresses both \emph{which} policy is the
dominant cause and \emph{how robustly} that causal claim holds across
...
and $\Pr(\mathrm{overshoot}>24\mid\mathrm{necessity})$ (right) when we fix
the partner-policy efficiency variable as a witness versus leave it free.
