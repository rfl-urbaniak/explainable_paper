...
promised by the PSCM definition above, with indicator-threshold mechanisms against
uniform exogenous noise:
\begin{align*}
\varname{gender}        &= \mathbb{1}[U_g \le \alpha_g], \\
\varname{credit}        &= \mathbb{1}[U_c \le \alpha_c], \\
\varname{check}         &= \mathbb{1}\bigl[U_{ck} \le \gamma_F(1-\varname{gender}) + \gamma_M\varname{gender}\bigr], \\
\varname{check\text{-}failed}  &= \varname{check}(1-\varname{credit}), \\
\varname{loan\text{-}if\text{-}checked} &= \mathbb{1}\bigl[U_\ell \le \mathrm{loan\text{-}prob}(\varname{gender}, \varname{check\text{-}failed})\bigr], \\
\varname{loan}          &= \varname{loan\text{-}if\text{-}checked} \cdot \varname{check},
\end{align*}
with $U_g, U_c, U_{ck}, U_\ell \overset{\text{iid}}{\sim} \mathrm{Unif}[0,1]$ as the
exogenous noise. For the running example we take $\alpha_g = \alpha_c = 0.5$,
...
$\mathbf{T}$ leaves the claim intact and merely weakens the context held
fixed --- a weaker probe of the same hypothesis, not a different one.
...
icardNormalityActualCausal2017}: factual upstream values play the role of the
\emph{default} against which counterfactual alternatives are graded.
...
Each reads as an informed relaxation of one of the three properties above, not an
unconsidered default --- exactly the user-facing knob the framework is designed to
expose.
...
same metric underlying Average Treatment Effect,
 Conditional Treatment Effect, and SHAP.
