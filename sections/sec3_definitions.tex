...
independent-product default of the remark below, the score they define reduces to a
short Monte Carlo loop:

\begin{algorithm}[htbp]
\caption{Monte Carlo estimator for $\mathbb{E}[ci]$ (independent-product default)}
...
\For{$j=1,\dots,N$}
  ...
  \State draw $(\mathbf{C}^s_j,\mathbf{T}^s_j)\sim\Gamma$ and, independently,
         $(\mathbf{C}^n_j,\mathbf{T}^n_j)\sim\Gamma$, both conditioned on
         $X_k$ being in the drawn suspect set \Comment{independent copy of
         $\Gamma$ per world, as in the remark after Def.~\ref{def:jointnecsuf}}
  \State draw $\mathbf{c}'_j\sim\Delta_{\mathbf{C}^n_j}(\mathbf{s}^\star)$
  \State $y^s_j \gets M.\mathrm{run}\bigl(\mathbf{u}_j,\ \mathrm{do}(\mathbf{C}^s_j=\mathbf{s}^\star\!\mid_{\mathbf{C}^s_j},\ \mathbf{T}^s_j=\mathbf{w}^\star\!\mid_{\mathbf{T}^s_j})\bigr)$ \Comment{sufficiency world}
  \State $y^n_j \gets M.\mathrm{run}\bigl(\mathbf{u}_j,\ \mathrm{do}(\mathbf{C}^n_j=\mathbf{c}'_j,\ \mathbf{T}^n_j=\mathbf{w}^\star\!\mid_{\mathbf{T}^n_j})\bigr)$ \Comment{necessity world, same $\mathbf{u}_j$}
  \State $ci_j \gets ci(y^s_j, y^n_j, y^\star)$
\EndFor
\State \Return $\Pr_\Gamma[X_k\in\mathbf{C}]^2\cdot\frac{1}{N}\sum_j ci_j$
\Comment{rescales the conditional mean back to $\mathbb{E}[ci]$'s sub-probability normalisation}
\end{algorithmic}
\end{algorithm}

\noindent In words: draw noise $\mathbf{u}$; draw a suspect/witness configuration
containing $X_k$ for each of the two worlds (independently); run the model twice under
the \emph{same} $\mathbf{u}$, once restoring the suspects to factual (sufficiency) and
once setting them to a fresh alternative (necessity); score the pair with $ci$; average.
