\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/structure_map_02_definitions.pdf}
  \caption{\textbf{Roadmap: defining \ourapproach} (current stage). With the
  motivations in place, the next section introduces the framework --- the variable
  selection distribution $\Gamma$, the alternative-value distribution $\Delta$, and the
  causal impact function $ci$. It then positions \ourapproach against the
  SHAP family (\cref{sec:shap_examples}), against actual causality and its
  probabilistic wrappers, and closes with the empirical evaluation.}
  \label{fig:roadmap_definitions}
\end{figure}
...
The development unfolds as follows. We write down seven \emph{desiderata} that any
responsibility attribution for Alice and Bob should satisfy, so that the formal
construction can be judged against an explicit target. We introduce the probabilistic
...
(Figure~\ref{fig:dag_obcb}). To keep the model a
...
unconsidered default --- exactly the user-facing choice the framework is designed to
make explicit.
...
held) giving $Y^s$. Conditional on $\mathbf{u}$ the two worlds are independent
(the product structure of Definition~\ref{def:jointnecsuf}); the impact kernel
$ci$ scores the pair, and its expectation over $\Gamma\otimes\Delta\otimes
P_{\mathbf{U}}$ is the \ourapproach{} score
(Definition~\ref{def:causalimpact}). The strip grounds the schematic in the
OBCB running example. \textbf{(b)} The impact kernel on the
$(Y^n,Y^s)$ plane, shown for the Absolute Difference score
$ci=|y^n-y^\star|-|y^s-y^\star|$ --- \emph{one} admissible choice of kernel
(the PNS binary score and necessity-only variants are others). The factual
reference lines $Y^n=y^\star$ (necessity) and $Y^s=y^\star$ (sufficiency) cross
at the factual outcome; $ci$ is largest at the ``ideal cause'' corner where the
alternative drives the outcome far from $y^\star$ while restoring the cause
sustains it. Sampled draws and their mean $\mathbb{E}[ci]$ illustrate the
...
counterfactual worlds in panel~(a), the kernel that scores them in panel~(b)). The choice is not arbitrary:
