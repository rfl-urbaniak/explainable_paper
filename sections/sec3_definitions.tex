...
samples antecedents and witnesses separately and then enforces the constraint
...
\frac{p_s^\Gamma(\mathbf{C})\,p_w^\Gamma(\mathbf{T})\,
\mathbb{I}[\mathbf{T}\cap\mathbf{C}=\emptyset]}{Z},
\qquad
Z \;=\; \sum_{(\mathbf{C}, \mathbf{T})} p_s^\Gamma(\mathbf{C})\,p_w^\Gamma(\mathbf{T})\,
\mathbb{I}[\mathbf{T}\cap\mathbf{C}=\emptyset].
\]
The normalizer $Z$ has a closed-form expression: conditioning on $\mathbf{T}$,
$Z = \mathbb{E}_{\mathbf{T}\sim\Gamma_w}\!\bigl[\Pr_{\mathbf{C}\sim\Gamma_s}
[\mathbf{C}\subseteq\mathbf{S}\setminus\mathbf{T}]\bigr]$, a finite sum over
$2^{|\mathbf{W}|}$ terms (and analogously by conditioning on $\mathbf{C}$); no
Monte Carlo is needed to evaluate it. Throughout, $\Gamma_s$ and $\Gamma_w$ are
...
and $\mathbf{W}$, the rejection step is vacuous, $Z = 1$, and $\Gamma$ reduces
to the product of marginals. In larger suspect sets the choice of cardinality
...
to isolate; we return to this design decision in the experimental section.
...
$(C,T)$ combinations each with $p^\Gamma(C,T)=\tfrac{1}{3}\cdot\tfrac{1}{2}=\tfrac{1}{6}$
