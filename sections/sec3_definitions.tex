...
The development proceeds in five steps: (1) write down a list of seven
\emph{desiderata} that any responsibility attribution for Alice and Bob should
satisfy, so that the formal construction can be judged against an explicit
target; (2) introduce the probabilistic structural causal model (PSCM) and
interventions, the substrate on which everything else rests; (3) introduce the
two distributions \ourapproach exposes to the user --- the variable selection
distribution $\Gamma$ over suspect/witness pairs and the alternative-value
distribution $\Delta$ over counterfactual cause settings; (4) combine them into
the joint necessity-sufficiency measure $P_k^{s,n}$ and the user-chosen causal
impact function $ci$ whose expectation is the \ourapproach score; (5) compute
the score on the OBCB example, verify that all seven desiderata hold, and
contrast \ourapproach's necessity/sufficiency decomposition with Pearl's
PN/PS/PNS. The closing paragraph then points forward to
Section~\ref{sec:shap_examples}, which positions \ourapproach against SHAP
and Causal SHAP.
...
given model. Restricting Shapley to $K<|N|$ would break the efficiency axiom, so the
choice is hardcoded into the framework rather than treated as a tunable. \ourapproach{}
surfaces the same decision as an explicit knob.}
...
to isolate; we return to this issue in Section~\ref{sec:ac_benchmark}, where
...
factual values (possibly with some excised regions around the factual values, read on for details)--- the construction we use throughout --- alternatives are weighted by
...
\medskip\noindent Three alternatives to upstream-only conditioning were considered and
not adopted as the default. \emph{Uniform} alternatives over $\dom(\mathbf{X})$ assign
equal density to every non-factual value regardless of context, including values the
structural model itself rules near-zero --- unrealistic and potentially incompatible
with $M$. \emph{Fully-conditional} alternatives, conditioned on every observed value
including downstream evidence, leak the consequence back into the counterfactual:
$\Delta(\mathbf{x})$ would be tightened by knowledge of the very outcome whose causes
we are interrogating. \emph{Marginal} (empirical) alternatives drop covariate
dependence entirely, producing alternatives that need not be plausible given the rest
of the factual instance. The framework permits any of these --- $\Delta$ is
user-pluggable --- but we recommend upstream-only as the principled default.
...
operationalise what \emph{different} means. Three natural choices for $\Delta$ are
available. The observational marginal $p(\mathbf{x}')$ --- as in kernel SHAP
...
\paragraph{Where we are.} The PSCM and intervention apparatus give us a notion of
...
sub-probability integral throughout. Suspect configurations with
...
total). Bob's PS for gender is undefined because the conditioning event
...
evaluation (Section~\ref{sub:synthetic}), and the SIR benchmark
(Section~\ref{sec:sir_benchmark}).
...
\item \textbf{Witness-mediated symmetry breaking.} Holding intermediate variables fixed
  ...
  Causal SHAP comparison), Section~\ref{sub:synthetic} (synthetic overdetermination
  and undercutting archetypes), and Section~\ref{sec:ac_benchmark} (witness ablation
