...
\varname{check}         &\sim \mathrm{Bernoulli}\bigl(\gamma_F(1-\varname{gender}) + \gamma_M\,\varname{gender}\bigr), \\
\varname{check\text{-}failed}  &= \varname{check}\,(1-\varname{credit}), \\
\varname{loan\text{-}if\text{-}checked} &\sim \mathrm{Bernoulli}\bigl(\mathrm{loan\text{-}prob}(\varname{gender}, \varname{check\text{-}failed})\bigr), \\
...
\varname{loan} are deterministic functions of the nodes above them. To keep the model a
PSCM, each draw $V \sim \mathrm{Bernoulli}(p)$ is realised as $V = \mathbb{1}[U_V \le p]$
with $U_V \overset{\text{iid}}{\sim} \mathrm{Unif}[0,1]$; it is this exogenous $U_V$,
held fixed across the factual and counterfactual worlds, that makes counterfactuals
well-defined. For the running example we take $p_g = p_c = 0.5$,
