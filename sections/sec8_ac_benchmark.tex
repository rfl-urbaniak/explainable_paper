...
flipping it changed the outcome, and at each site the higher-scoring of the two rivals
$A_i, B_i$ is returned as the cause. The procedure transcribes the \texttt{ThinSearchSampler}
...
The active suspect set $\mathbf{C}_j$ (line~10) is drawn at uniformly chosen
cardinality $k_s \in \{1,\dots,n_s\}$; the dropped witness set $D_j$
(line~11) is drawn the same way at $k_w \in \{0,\dots,n_w\}$, and the active
...
witness ablation flag $\mathrm{wit}=\text{off}$ is handled once on line~2
...
which lets the per-suspect score on lines~16--18 be read off as the
...
sufficiency active is exercised in Appendices~\ref{sub:synthetic}, \ref{sec:DCE}, and~\ref{sec:sir_benchmark}, where AC has no comparable
...
the two configurations is what the ablation measures. The conceptual case for
